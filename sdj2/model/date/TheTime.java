...
public class TheTime implements Serializable {
	...
	private static final long serialVersionUID = -4222571365842631543L;
	private int day;
	private int month;
	private int year;
	 ...
	 * 
	 * @return myCurrentDate the current date
	 */
	...
	public String dayOfWeek() {
		...
		String dayofweek;
		int dayoftheweek = (day + ((13 * (month + 1)) / 5) + (year % 100) + ((year % 100) / 4) + ((year / 100) / 4)
				+ 5 * (year / 100)) % 7;
	 ...
	 * @param date2
	 *            the date to check
	 * @return true if the date is before and false if the date is after
	 */
