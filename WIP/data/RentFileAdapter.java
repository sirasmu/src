...
public class RentFileAdapter {
	 ...
	 * 
	 * @param fileName
	 *            the name and path of the file where rentals will be saved and
	 *            retrieved
	 */
	...
	/**
	 * Uses the MyFileIO class to retrieve a ReservedList object with all Rents.
	 * 
	 * @return rentals a ReservedList object with all stored rentals
	 */
	public ReservedList getAllRents() {
		ReservedList rentals = new ReservedList();

		try {
			rentals = (ReservedList) mfio.readObjectFromFile(fileName);
	 ...
	 * @return rentals a ReservedList object with rentals from the pick up date
	 */
	public ReservedList getRentsFromPickUpDate(int day, int month, int year) {
	 ...
	 * @return rentals a ReservedList object with rentals from the pick up date
	 */
	public ReservedList getRentsFromPickUpDate(int month, int year) {
	 ...
	 * @return rentals a ReservedList object with rentals by their status
	 */
	public ReservedList getReservedListByTypeStatus(String status) {
		ReservedList rentals = new ReservedList();

		ReservedList result = getAllRents();
		for (int i = 0; i < result.size(); i++) {
	 ...
	 */
	public void saveRents(ReservedList rentals) {
		try {
	...
	/**
	 * Uses the MyFileIO class to change the first name, last name, pick up
	 * time, return time, pick up place, return place, estimated kilometers and
	 * vehicle of a rent with the given reservation number.
	 * 
	 * @param resNo
	 ...
	 * @param estimateKm
	 *            the estimated amount of kilometers the client will drive in
	 *            the vehicle
	 * @param vehicle
	 *            the vehicle the client books
	 */
	public void changeBooking(int resNo, String firstName, String lastName, TheTime pickUpTime, TheTime returnTime,
			String pickUpPlace, String returnPlace, int estimateKm, Vehicle vehicle) {
		ReservedList rentals = getAllRents();
	 ...
	 *            the new real price
	 */
	public void changeReturn(int resNo, TheTime realDate, int drivenKm, String condition, double realPrice) {
		ReservedList rentals = getAllRents();
		for (int i = 0; i < rentals.size(); i++) {
			Rent rent = rentals.get(i);
	 ...
	 * @param returnTime2
	 *            the return time of the vehicle
	 * @return list the list with vehicle that are available for a specific
	 *         period of time
	 */

	public VehicleList timeIsAvailable(VehicleList list, TheTime pickUpTime2, TheTime returnTime2) {
		ReservedList rentals = getAllRents();
		for (int i = 0; i < rentals.size(); i++) {
			Rent rent = rentals.get(i);
			if (rent.getPickUpTime().equals(pickUpTime2) || rent.getReturnTime().equals(returnTime2)
					|| rent.getPickUpTime().equals(returnTime2) || rent.getReturnTime().equals(pickUpTime2)) {
				list.remove(rent.getVehicle());
			}

			else if (rent.getPickUpTime().isBefore(pickUpTime2) && pickUpTime2.isBefore(rent.getReturnTime())) {
				list.remove(rent.getVehicle());
			} else if (pickUpTime2.isBefore(rent.getPickUpTime()) && rent.getPickUpTime().isBefore(returnTime2)) {
				list.remove(rent.getVehicle());
			} else if (rent.getPickUpTime().isBefore(pickUpTime2) && returnTime2.isBefore(rent.getReturnTime())) {
				list.remove(rent.getVehicle());
			} else if (pickUpTime2.isBefore(rent.getPickUpTime()) && rent.getReturnTime().isBefore(returnTime2)) {
				list.remove(rent.getVehicle());
			}
		}
		return list;
	...
	public String placeIsAvailable(String pickUpPlace, String returnPlace) {
		...
		for (int i = 0; i < rentals.size(); i++) {
			...
			if (rent.getPickUpPlace().equals(pickUpPlace) && rent.getReturnPlace().equals(returnPlace)) {
				return "ok place";
			}
			...
			if (!(rent.getPickUpPlace().equals(pickUpPlace)) && (rent.getReturnTime().equals(returnPlace))) {
	 ...
	 * @return temp a list of all rents for selected vehicle
	 */
	public ReservedList getAllRentsForVehicle(Vehicle vehicle) {
